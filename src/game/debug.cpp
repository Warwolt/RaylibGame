...
void render_debug_overlay(const Game& game) {
	switch (game.debug.reload_state.value()) {
		case HotReloadState::Rebuilding: {
			const Animation<std::string> animation = {
				{ "Rebuilding", 500ms },
				{ "Rebuilding.", 500ms },
				{ "Rebuilding..", 500ms },
				{ "Rebuilding...", 500ms },
			};
			const Time start_time = game.debug.reload_state.last_changed();
			const std::string text = get_animation_value(animation, start_time, Time::now());
